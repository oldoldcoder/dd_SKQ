		...
		for (auto it = tmp->hashmap_forward->begin(); it != tmp->hashmap_forward->end(); ++it) {
			...
			for (int j = 0; j < len; ++j) {
				...
				if (itt == tmp->hashmap_backward->end()) {
					...
					vector<int>* fileIds = (vector<int>*)itt->second;
					fileIds->push_back(file_id);
				}
			}
		}
...
query* read_query(char* queryFilePath) {
	...
	for (int i = 0; i < q->n; ++i) {
		std::string line;
		q->units[i] = new  query_unit();
		if (std::getline(infile, line)) {
			std::istringstream iss(line);
			iss >> q->units[i]->target_id;
			// 获取剩余部分作为keyword，并去除前后的空白字符
			std::string keyword;
			std::getline(iss >> std::ws, keyword); // 跳过前导空白字符
			q->units[i]->keyword = keyword; // 调用trim函数
		}
	}
